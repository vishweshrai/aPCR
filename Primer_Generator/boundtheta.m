function theta = boundtheta(seq,pricon,plascon,monv,biv,tann,a,d,g,catrat)
    ...
    Ka = exp(-(h-tann*s)/(1000*R*tann));
    theta = 1-((Ka*(plascon-pricon)-1)/(2*Ka*plascon)+sqrt(Ka^2*(pricon-plascon)^2+2*Ka*(pricon+plascon)+1)/(2*Ka*plascon));
    % fprintf('Fraction of bound strands is %f!',theta);
end